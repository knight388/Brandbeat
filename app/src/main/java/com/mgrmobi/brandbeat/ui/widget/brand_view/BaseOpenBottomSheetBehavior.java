...
import com.mgrmobi.brandbeat.ui.widget.brand_view.utils.MathUtils;
 ...
 */
public class BaseOpenBottomSheetBehavior  <V extends View> extends CoordinatorLayout.Behavior<V> {
     ...
     * @hide
     */
    @IntDef({STATE_EXPANDED, STATE_COLLAPSED, STATE_DRAGGING, STATE_SETTLING, STATE_HIDDEN, STATE_MIDDLE})
    ...
    protected int mMiddleOffset;

    protected int mMiddleOffsetDivider;

    protected boolean mHideable;

    @State
    protected int mState = STATE_MIDDLE;
    ...
    protected BottomSheetCallback mCallback;
     ...
     */
    public BaseOpenBottomSheetBehavior() {
    }
     ...
     */
    public BaseOpenBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BaseOpenBottomSheetBehavior_Params);
        setPeekHeight(a.getDimensionPixelSize(R.styleable.BaseOpenBottomSheetBehavior_Params_behavior_custom_peekHeight, 0));
        setHideable(a.getBoolean(R.styleable.BaseOpenBottomSheetBehavior_Params_behavior_custom_hideable, false));
        mMinOffset = a.getDimensionPixelSize(R.styleable.BaseOpenBottomSheetBehavior_Params_behavior_custom_minOffset, NO_VALUE);
        mMiddleOffset = a.getDimensionPixelSize(R.styleable.BaseOpenBottomSheetBehavior_Params_behavior_custom_middleOffset, NO_VALUE);
        mMiddleOffsetDivider = a.getDimensionPixelSize(R.styleable.BaseOpenBottomSheetBehavior_Params_behavior_custom_middleOffsetDivider, NO_VALUE);
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mMinOffset == NO_VALUE) {
            mMinOffset = Math.max(0, mParentHeight - child.getHeight());
        }
        mMaxOffset = Math.max(mParentHeight - mPeekHeight, mMinOffset);

        if (mMiddleOffset == NO_VALUE) {
            if (mMiddleOffsetDivider != NO_VALUE) {
                mMiddleOffset = (mMaxOffset - mMinOffset) / mMiddleOffset;
            } else {
                mMiddleOffset = (mMaxOffset - mMinOffset) / 2;
            }
        }
    ...
    public void setMaxOffset(int value) {
        mMaxOffset = value;
    }
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        int top;
        int targetState;
        if (target.getTop() > mMiddleOffset && mLastNestedScrollDy > 0) {
            top = mMiddleOffset;
    ...
    protected boolean shouldHide(View child, float yvel) {
    ...
    public static <V extends View> BaseOpenBottomSheetBehavior<V> from(V view) {
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if (!(params instanceof CoordinatorLayout.LayoutParams)) {
            throw new IllegalArgumentException("The view is not a child of CoordinatorLayout");
        }
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params).getBehavior();
        if (!(behavior instanceof BaseOpenBottomSheetBehavior)) {
            throw new IllegalArgumentException("The view is not associated with BottomSheetBehavior");
        }
        return (BaseOpenBottomSheetBehavior<V>) behavior;
    }
}
